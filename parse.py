...
_SERVICE_SCHEMA = {
    ...
    'properties': {
        'methods': {
            'type': 'object',
            'patternProperties': {
                _IDENTIFIER_PATTERN: _RPC_SCHEMA
            },
            'additionalProperties': False
        }
    },
...
_MESSAGE_SCHEMA = {
    ...
    'properties': {
        'fields': {
            'type': 'object',
            'patternProperties': {
                _IDENTIFIER_PATTERN: _FIELD_SCHEMA
            },
            'additionalProperties': False
        },
        'nested': {
...
def _parse_service(service_name: str, service_spec: dict[str, dict]) -> str:
    ...
    for method_name, method_spec in service_spec['methods'].items():
        ...
        line = (method_name, f'  rpc {method_name} ({request_type}) returns ({response_type});')
        lines.append(line)
...
def _parse_message(indent: int, message_name: str, message_spec: dict[str, dict]) -> str:
